...
- 下载页
  ...
  - "下载中"列表项显示章节下载进度
  - **压缩下载**， 保存压缩章节漫画文件: **漫画名(文件夹)\章节链接名称.zip**  <br />
    **直接下载**，保存图片文件: **漫画名(文件夹)\章节链接名称(文件夹)\图片序号.jpg** <br />
    **拼接下载**，保存图片文件: **漫画名(文件夹)\章节链接名称(文件夹)\拼接后图片序号.jpg**  <br />
    **批量下载**，保存图片文件: **漫画名(总文件夹)\#001 / Vol.001(子文件夹)\图片序号.jpg**  <br />
    如要保存在 ***文件夹*** 中，需要设置 **Tampermonkey [浏览器 API](#browserapi)**
...
## 2023.2.7 
  从 **1.4.0** 版本起脚本更改为任意网页运行，脚本默认不加载界面，可通过快捷键 **Alt + V** 加载界面，或点击浏览器 Tampermonkey 扩展图标中的“加载UI” <br />
  当然也可以在脚本设置中恢复默认加载界面 （**不建议**。1、部分网站为SPA页面，页面更改不会重新触发脚本。2、脚本使用 Vant 组件库, 可能影响原网站样式，如会改变网站的字体大小等)
<br /><br />

## 操作指北
 - <sapn id='browserapi'> Tampermonkey - 浏览器 API（设置）
    - Tampermonkey - 设置 - 通用 - 配置模式 - 选高级
    - 设置 - 下载 BETA - 下载模式 - 选浏览器API
  - **唤不起脚本界面?**
    - **原因**  脚本引入外部组件，可能会有因不同地区的网络影响导致没有加载的这些外部资源，使脚本无法正常运行。<br />
    具体点击 Tampermonkey 扩展，右键“10图漫”脚本，进入脚本编辑页后，点击外部，查看是否有 4 个外部引用链接缓存（文件大小不为0
    <br />
...
## <font color="#dd0000">* 声明/注意</font>
  ...
  - 如压缩下载时有较多 Tampermonkey 跨域弹窗提示，建议直接下载
<br /><br />
